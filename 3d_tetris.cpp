...
bool loadObjFile(const std::string& filename, std::vector<point_3d>& vertices, std::vector<std::vector<int>>& faces, std::vector<std::vector<int>>& lines) {
...
void draw_stage(sf::RenderWindow& window, std::vector<point_3d>& vertices, std::vector<std::vector<int>>& faces) {
...
bool compareFaces(const std::vector<int>& face1, const std::vector<int>& face2, const std::vector<point_3d>& vertices) {
...
void draw_world(sf::RenderWindow& window, std::vector<point_3d>& vertices, std::vector<std::vector<int>>& faces) {
...
void write_cube(std::vector<point_3d>& vertices, float x, float y, float z) {
	vertices.push_back(point_3d(x, y, z));
	...
	for (int i = 0; i < 8; i++) {
		point_3d& vertex = vertices[vertices.size() - 1 - i];
		vertex.x -= 1.5;
		vertex.y -= 1.5;
		vertex.z += 1.5;
	}
...
void generate_world_blocks(int world[10][3][3], std::vector<point_3d>& vertices, std::vector<std::vector<int>>& faces) {
	vertices.clear();
	faces.clear();
	int base_idx = 0;
...
void generate_blocks(int blocks[7][3][3][3], int kind, std::vector<point_3d>& vertices, std::vector<std::vector<int>>& lines) {
	...
	std::vector<std::pair<point_3d, point_3d>> overlapping_edges;
...
int main() {
	...
	while (window.isOpen()) {
		...
		//인티저회전
		{
			...
			const int ANI_STEP = 100;
			auto smooth_draw = [&](point_3d angle, point_3d center, point_3d center_to) {
			...
			auto smooth_rotateY = [&](float angle)
			{
				...
				get_minmax(); point_3d center((minx + maxx) / 2.0 - 1, 0, (minz + maxz) / 2.0 + 2);
				smooth_draw({ 0,angle,0 }, center, center_to);
			};

			auto smooth_rotateX = [&](float angle)
			{
				get_minmax(true); point_3d center_to(0, (miny + maxy) / 2.0 - 1.0, (minz + maxz) / 2.0 + 2);
				get_minmax(); point_3d center(0, (miny + maxy) / 2.0 - 1, (minz + maxz) / 2.0 + 2);  // 객체의 중심점 정의
				for (int step = 0; step < ANI_STEP; step++)
				{
					window.clear();
					draw_stage(window, vs, fs);
		...
		if (game_over) {
			...
			text.setPosition(point_2d(window.getSize().x / 2.0f, window.getSize().y / 2.0f));
			window.draw(text);
		}
		else {
