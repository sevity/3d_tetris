...
int main() {
	...
	while (window.isOpen()) {
		...
		//인티저회전
		{
			...
			{// animation here
				get_minmax(true);
				sf::Vector3f center_to((minx + maxx) / 2.0 - 1, 0, (minz + maxz) / 2.0 + 2);  // 객체의 중심점 정의

				get_minmax();
				sf::Vector3f center((minx + maxx) / 2.0 - 1, 0, (minz + maxz) / 2.0 + 2);  // 객체의 중심점 정의
				printf("center: %f, %f, %f, center_to: %f, %f, %f\n", center.x, center.y, center.z, center_to.x, center_to.y, center_to.z);
				//printf("minx:%d, maxx:%d, miny:%d, maxy:%d\n", minx, maxx, miny, maxy);
				for (int step = 0; step < ANI_STEP; step++)
				{
			...
			{// animation here
				get_minmax(true);
				sf::Vector3f center_to(0, (miny + maxy) / 2.0 - 1, (minz + maxz) / 2.0 + 2);  // 객체의 중심점 정의

				get_minmax();
				sf::Vector3f center(0, (miny + maxy) / 2.0 - 1, (minz + maxz) / 2.0 + 2);  // 객체의 중심점 정의
				printf("center: %f, %f, %f, center_to: %f, %f, %f\n", center.x, center.y, center.z, center_to.x, center_to.y, center_to.z);
				//printf("minx:%d, maxx:%d, miny:%d, maxy:%d\n", minx, maxx, miny, maxy);
				for (int step = 0; step < ANI_STEP; step++)
				{
