...
bool loadObjFile(const std::string& filename, std::vector<sf::Vector3f>& vertices, std::vector<std::vector<int>>& lines) {
...
int main() {
    ...
    loadObjFile("output.obj", vertices, lines);
        ...
        window.clear();
         ...
         for (const auto& line : lines) {
            sf::Vertex lineVertices[] = {
                sf::Vertex(project(vertices[line[0]]) * 400.f + sf::Vector2f(400, 300)),
                sf::Vertex(project(vertices[line[1]]) * 400.f + sf::Vector2f(400, 300))
            };
